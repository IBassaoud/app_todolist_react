...
class CreateTaskComponent extends React.Component {
    ...
    inputDeleteKey = React.createRef();
    ...
    handleDeleteTask = (key) => () => {
        ...
        // this.setState({ inputDeleteKey: this.state.inputDeleteKey.current.keyelement });
        // console.log(this.state.inputDeleteKey)
        // let newTask = {
        //     name: this.state.inputTask.current.value,
        //     state: "status-todo",
        //     stateText: "Todo",
...
function RenderCreatedTaskComponent(props) {
    return (
        <tr>
            <td className="text-center task_name">{props.task.name}</td>
            <td>
            ...
            <td className="actions">
                <div className="actions_buttons">
                    ...
                    <div className="actions_item" onClick={props.deletetask}>
                        <input type="hidden" name={props.task.name} ref={props.delElement} value={props.task.key} />
                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" />
                        </svg>
                    </div>
